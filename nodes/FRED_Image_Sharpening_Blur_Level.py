...
class FRED_Image_Sharpening_Blur_Level:
    ...
    def execute(self, image, mask_optional=None, minimum_deblur_output=50.0):
        ...
        print("[FRED_Image_Sharpening_Blur_Level] Could not reach minimum_deblur_output, passing original image.")
        self.print_tensor_info(image, "Original Input Image")
        # return (image,)
        output = self.fix_output(image, image)
        return (output.to(device=device, dtype=dtype).clamp(0, 1).contiguous(),)
...
# class FRED_Image_Sharpening_Blur_Level:
    # """
    # Adaptive sharpening to reach minimum_deblur_output.
    # Supports optional mask for blur measurement.
    # Prints raw and scaled blur values for debug and verification.
    # """
...
# # Inside execute function before return:
# print_tensor_info(output_image, "Output Image")
        # return (image,)
